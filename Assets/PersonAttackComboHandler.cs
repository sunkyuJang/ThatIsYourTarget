...
public class PersonAttackConditionHandler : AttackConditionerHandler
{
    public PersonAttackConditionHandler(AnimatorController animatorController) : base(animatorController) { }
    ...
    public override AnimationComboStateNode GetInitiatedNode(RequireData requireData)
    {
        AnimationComboStateNode node = null;
        ...
        node = AttackingComboManager.GetStartNode();
        node = FindNextNode(requireData, node, null);
        return node;
    }

    protected override AnimationComboStateNode FindNextNode(RequireData requireData, AnimationComboStateNode node, Action<Animator> parametterSetter)
    {
    ...
    protected bool IsFindNextNodeNameFromStart(PersonRequireData data, AnimationComboStateNode node, ref Action<Animator> action, out AnimationComboStateNode nextNode)
    {
        //initiate node
        nextNode = node;
        ...
        for (int i = 0; i < node.nextAnimations.Count; i++)
        {
            ...
            if (shouldAdd)
            {
                ...
                nextNode = AttackingComboManager.GetStateNode(node.nextAnimations[i]);
                return true;
            }
        }
    ...
    bool IsFindNextNodeNameFromCustom(PersonRequireData data, AnimationComboStateNode node, ref Action<Animator> actions, out AnimationComboStateNode nextNode)
    {
        nextNode = node;
        return false;
    }

    bool IsFindNextNodeNameFromLoop(PersonRequireData data, AnimationComboStateNode node, ref Action<Animator> action, out AnimationComboStateNode nextNode)
    {
        nextNode = node;
    ...
    int SetFunc(AnimationComboStateNode node)
    {
        return 0;
    }
