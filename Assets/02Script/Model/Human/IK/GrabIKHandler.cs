...
public class HumanHandLimbIKSetter : MonoBehaviour
{
    ...
    [SerializeField] private float limbPositionWeight = 0f;
    [SerializeField] private float maxLimbPositionWeight = 0.2f;
    [SerializeField] private float limbRotationWeight = 0f;
    [SerializeField] private float maxLimbRotationWeight = 0.5f;
    [SerializeField] private float fingerWeight = 0f;
    [SerializeField] private float maxFingerWeight = 1f;
    ...
    {
        var startLimbPositionWeight = limbPositionWeight;
        var startLimbRotationWeight = limbRotationWeight;
        var startFingerWeight = fingerWeight;
        ...
        while (elapsedTime <= during)
        {
            ...
            limbPositionWeight = Mathf.Lerp(startLimbPositionWeight, maxLimbPositionWeight, elapsedTime / during);
            limbRotationWeight = Mathf.Lerp(startLimbRotationWeight, maxLimbRotationWeight, elapsedTime / during);
            fingerWeight = Mathf.Lerp(startFingerWeight, maxFingerWeight, elapsedTime / during);

            // Update IK weights here
            limbIK.solver.IKPositionWeight = limbPositionWeight;
            limbIK.solver.IKRotationWeight = limbRotationWeight;

            if (fingerPositioner != null)
            {
                fingerRig.weight = fingerWeight;
            }
